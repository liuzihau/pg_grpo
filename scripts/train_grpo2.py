...
from src.data.pregen_kd_ds import PreGeneratedTopKDataset   # keep if you use it elsewhere
from src.data.grpo_pregen import collate_grpo_prefixes
from src.data.prompts import load_prompts_for_split
...
def _gather_token_logps_from_forward(
    ...
    # full next-token log-probs at those positions
    logp_next = logp_all.gather(1, pos_logits.unsqueeze(-1).expand(-1, -1, V))  # [N,T,V]
...
def main():
    ...
    while step < max_steps:
        for batch in train_loader:
            ...
            # 3) Draft logprobs on sampled tokens WITH grad
            ...
            # 4) Teacher logprobs on the same sampled tokens (no grad)
            ...
            # 5) Optional ref logprobs for KL
            if kl_ref == "teacher":
            ...
            # 6) GRPO-style loss (placeholder — plug your actual loss)
...
def _run_validation_once(draft, teacher, tokenizer, val_ds, cfg, device, max_new, temperature,
    ...
    for batch in val_loader:
        ...
        if acceptance_source == "full_overlap":
            ...
            _ = ((q_mask > 0) & (p_mask > 0))  # valid_mask if you compute metrics
        else:
            with torch.autocast("cuda", dtype=torch.bfloat16):
                seqs_in = _on_right_device_for_generate(draft, seqs)
                att_in  = _on_right_device_for_generate(draft, att)
            ...
            _ = ((gen_mask_NT > 0) & (t_mask_NT > 0))  # valid_mask if you compute metrics
